...
def neighbors_generator_v3(radius_inner, radius_outer, length_part, degree_delta):
    result = set()
    lengths = []
    degrees = []
    for i in range(length_part):
        lengths.append(random.randint(radius_inner, radius_outer))
    # lengths = [radius_inner, radius_outer, int((radius_inner+radius_outer)/2)]
    degree_part = int(360/degree_delta)
    for i in range(degree_part):
        degrees.append(random.randint(0, 360))
...
class pathfinder(AStar):
    ...
    def __init__(self, ver, gridMap, neigh_range, length_part=10, degree_delta=10, roads=None, com_lines=None, openset_size=800):
        ...
        self.neighbors_coordinate = neighbors_generator_v3(neigh_range[0], neigh_range[1], length_part, degree_delta)
    ...
    def neighbors(self, node):
        ...
        filter3 = [j for j in filter2 if not self.is_forbiddenzoom_in_between(node, j)]
        return filter3
